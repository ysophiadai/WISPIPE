...
;###############################################################
PRO match_cat_f160,field,path0
...
;det='2.5'  ;<--- in certain test fields
...
openw,u2,path+'SEX/F160_m.cat',/get_lun
openw,u3,path+'SEX/F110_um.cat',/get_lun
openw,u4,path+'SEX/F160_um.cat',/get_lun
...
if ind2[0] ne -1 then mag2(ind2) = limit_160 
if ind3[0] ne -1 then mag1(ind3) = limit_110
if ind4[0] ne -1 then mag2(ind4) = limit_160
...
match=0
       FOR j=0,N_ELEMENTS(F160_number)-1 DO BEGIN
...
d=SQRT( ((x1_w(i) - x2_w(j)))^2 + ((y1_w(i)-y2_w(j)))^2) 
             ...
             IF  SQRT( (x1_w(i) - x2_w(j))^2 + (y1_w(i)-y2_w(j))^2) LT dmax THEN BEGIN           
                ...
                printf,u2,F160_number(j),x2(j),y2(j),a2(j),b2(j),theta2(j),x2_w(j),y2_w(j),$
...
match2=0
       ...
       printf,u4,F160_number(j),x2(j),y2(j),a2(j),b2(j),theta2(j),x2_w(j),y2_w(j),$
       a2_w(j),b2_w(j),theta2_w(j),mag2(j),magerr2(j),star2(j),flag2(j),FORMAT='(15(F12.6))'
       ENDIF
ENDFOR
...
cat_F160_m=DDREAD(path+'SEX/F160_m.cat')
cat_F160_um=DDREAD(path+'SEX/F160_um.cat')
...
cat_F160_m=cat_F160_m[*,ind_F110_m]
ind_F160_um=sort(cat_F160_um[11,*])
cat_F160_um=cat_F160_um[*,ind_F160_um]

cat_F110_m[0,*]=fix(cat_F110_m[0,*])
cat_F110_um[0,*]=fix(cat_F110_um[0,*])
cat_F160_m[0,*]=fix(cat_F160_m[0,*])
cat_F160_um[0,*]=fix(cat_F160_um[0,*])
...
cat_F160_um[0,*]=2000+indgen(N_ELEMENTS(cat_F160_um[0,*]))+1;cat_F160_um[0,*]

;CONCAT MATHCED & UNMATCHED
;*****************************
cat_F110=[[cat_F110_m],[cat_F110_um],[cat_F160_um]]
cat_F160=[[cat_F160_m],[cat_F110_um],[cat_F160_um]]

cat_F110[11,where(cat_F110[0,*] ge 2000)]=limit_110
cat_F160[11,where(cat_F160[0,*] ge 1000 and cat_F160[0,*] lt 2000)]=limit_160
...
PRINTF,u2,cat_F160,FORMAT='((I5, 4(F10.3), F10.1, 4(F20.10), F10.1, 2F10.4, F10.2, I10))'

CLOSE,u1,u2
FREE_LUN,u1,u2


seflag,'F160W',det,field,path0,/blend
