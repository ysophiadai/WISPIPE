...
;#        beam
;# procedure called: 
;# Output: to generate the cutput .eps files in
;#         path+'Plot/' folder, which plots the G102 and G141 spectra
;# Note: Empty frames commonly seen in cases where the 'trimmed' spectra has no value.
;# updated by 
;# Sophia Dai 2014.05.06
;###############################################################

PRO axe_sing_F140,field,beam,n,shift,trim,SAVE=SAVE,path0
...
;       help, temp, /struct
         ...
         readcol,path+'DATA/DIRECT_GRISM/G102_0th.txt',g102_zx,g102_zy,g102_z_beam,g102_zmag,format=('f,f,f,f'),/silent 
...
;################################  GRISM G102   ##################################
  ...
  ; This return generated blank frames in the plot files, ignore!
  ;     if N_elements(G102_wave) eq 0 then return
   ...
   g102_zo=G102_flux*0.   
       ...
       for l=0,n_elements(g102_zx)-1 do begin
          ind_a=where(round(pet_a[*,0]) eq round(g102_zx(l)) and round(pet_a[*,1]) eq round(g102_zy(l)) and g102_zmag(l) lt 22.5)
...
ENDIF  ;correspond to     'IF (SPC_IND[0] NE -1)' in line 69
...
IF (SPC_IND[0] NE -1) and (SPC_IND2[0] NE -1) then ymax=max([G102_flux_rebin[5:n_elements(G102_flux_rebin)-15], G141_flux_rebin[5:n_elements(G141_flux_rebin)-15]])*1.2
IF (SPC_IND[0] NE -1) and (SPC_IND2[0] eq -1) then ymax=max(G102_flux_rebin[5:n_elements(G141_flux_rebin)-15])*1.2
IF (SPC_IND[0] eq -1) and (SPC_IND2[0] NE -1) then ymax=max(G141_flux_rebin[5:n_elements(G141_flux_rebin)-15])*1.2
IF (SPC_IND[0] eq -1) and (SPC_IND2[0] eq -1) then  begin
 plot,[0],[0],xrange=[7200,16500],yrange=[-1e-18,0]/1e-18,/xstyle,/ystyle,$
...
endIF
   ...
   XYOuts, x, ypos,'Object'+strcompress(beam)+TeXtoIDL('   H_{140}=')+STRING(magh[n],FORMAT='(F5.2)')$
   ...
   ;****************************
IF (SPC_IND[0] NE -1) then begin
oplot,G102_wave_rebin,G102_flux_rebin/1e-18,color=100,thick=3,psym=10
   ;oploterror,G102_wave_rebin,G102_flux_rebin,G102_ferr_rebin,thick=1,psym=3,/nohat
oplot,G102_wave_rebin,G102_contam_rebin/1e-18,thick=2,psym=10
...
IF (SPC_IND[0] NE -1) and (SPC_IND2[0] eq -1) then begin
           wave=[G102_wave_trim[0:dim1-(cut+1)]]
           flux=[G102_flux_trim[0:dim1-(cut+1)]]
           ferr=[G102_ferr_trim[0:dim1-(cut+1)]]
           contam=[G102_contam_trim[0:dim1-(cut+1)]] 
           zeroth=[g102_zo[0:dim1-(cut+1)]]
endif
